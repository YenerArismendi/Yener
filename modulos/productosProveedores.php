...
if(!empty($_POST["registrarProducto"])){
    if(empty($_POST["nombre-producto"]) or empty($_POST["pais-producto"]) or empty($_POST["fecha-producto"]) or empty($_POST["precio-unidad"]) or empty($_POST["advertencias-producto"]) or empty($_POST["descripcion-producto"])){
        ...
        $PrecioTotal =$_POST["precio-unidad"];
        $Advertencias = $_POST["advertencias-producto"];
        $Descripcion = $_POST["descripcion-producto"];

        //Almacenar datos en la bd 

        $sql=$conexionsql->query("INSERT INTO producto (Nombre, pais, fecha, advertencias, descripcion, PrecioUnidad) VALUES ('$NombreProducto', '$PaisProducto', '$FechaProducto', '$Advertencias', '$Descripcion', '$PrecioTotal')");
            if($sql){
